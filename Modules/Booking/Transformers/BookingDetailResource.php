...
class BookingDetailResource extends JsonResource
{
    ...
    {
        $service_amount = $this->booking_service ? $this->booking_service->sum('service_price') : 0;
        $couponAmount = UserCouponRedeem::where('booking_id', $this->id)->value('discount');
        $couponcut_amount = $service_amount - $couponAmount;
        $tax_details = getBookingTaxamount(($this->booking_service ? $this->booking_service->sum('service_price') : 0) + ($this->products ? $this->products->sum('discounted_price') : 0),($couponAmount), $this->payment ? $this->payment->tax_percentage : null);
        return [
            'id' => $this->id,
            'note' => $this->note,
